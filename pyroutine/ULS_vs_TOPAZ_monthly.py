'''ULS vs TOPAZ monthly plot/stats
'''
...
def draw_monthly_sit_uls(chrono_dt_uls, sit_uls, sit_std_uls, 
                 valid_ll_fr, sit_na_loc, sit_bl_loc, 
                 valid_ll, sit_ml_loc,
                 buoy='', rootdir='', savefig=False, showfig=False):
    '''Plot the monthly SIT for ULS FRAM, and TOPAZ4-ML, -BL, -FR
    '''
...
#     date_max = datetime.datetime(2010,12,31)
    ...
    # ULS
    plt.plot(chrono_dt_uls, sit_plt.where(sit_plt>0), c='k', ls='--', lw=2, label=f'{buoy}')  #ULS {buoy.upper()}')
    if sit_std_uls is not None:
        plt.fill_between(chrono_dt_uls, sit_uls.where(sit_uls>0)+sit_std_uls, sit_uls.where(sit_uls>0)-sit_std_uls, color='grey', alpha=0.2) #, where=~mask_gap)
    ...
    ofile = f'uls_{buoy.upper().replace(" ","")}_SIT_TOPAZ_{date_min.year}_{date_max.year}_01.png'
...
buoy_type = 'Fram'  # 'BGEP' / 'NPEO' / 'ADCP' / 'Fram'
...
if buoy_type == 'Fram':
    buoy = 'F11'
    ifile = getfile_uls_Fram(buoy=buoy)
...
sit_ml, chrono_ml, sit_na, sit_bl = merge_TOPAZ.load(return_na=True, return_bl=True)
...
draw_monthly_sit_uls(chrono_dt_uls, sit_sm, sit_std_sm, 
             valid_ll_fr, sit_na_loc, sit_bl_loc, 
             valid_ll, sit_ml_loc,
...
#              valid_ll_fr, sit_na_loc, sit_bl_loc, 
#              valid_ll, sit_ml_loc,
#              buoy=buoy_name, rootdir=rootdir, savefig=False, showfig=True)
